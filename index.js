...
function copy(flavorsArray) {
  const a = [...flavorsArray];
  return a;
}
...
function getFlavorByIndex(flavorsArray, flavorIndex) {
  return flavorsArray[flavorIndex];
}
...
function removeFlavorByName(flavorsArray, flavorName) {
  for (var i = 1; i < flavorsArray.length; i++) {
    if (flavorsArray[i] === flavorName) {
      flavorsArray.splice(i, 1);
      return flavorsArray;
    }
  }
}
